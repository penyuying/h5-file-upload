...
_Upload.prototype = {
     ...
     * @returns {file|null}
     */
    _fileReader: function _fileReader(file, index) {
        var _this = this,
            option = _this._options,
        ...
        reader.onload = function () {
             ...
             */
            function callback() {
                var data = _this._compress(img, file.type, index, file); // 压缩图片
     ...
     */
    _addFileBtn: function _addFileBtn(el) {
        var _this = this,
            option = _this._options;
        if (!el) {
            return;
     ...
     */
    _inArray: function _inArray(item, arr) {
        var res = -1;
     ...
     */
    _execUpload: function _execUpload(file, index) {
        var _this = this;
        try {
     ...
     */
    _getRatio: function _getRatio(width, height, option) {
        // 宽度压缩的倍数（水平像素数）
     ...
     */
    _compress: function _compress(img, type, index, file) {
        var _this = this,
            option = _this._options,
     ...
     * @returns {blob} 文件对象
     */
    _toBuffer: function _toBuffer(basestr, type, fileName) {
     ...
     * @param {Number} index 文件索引
     */
    _funUploadFile: function _funUploadFile(file, index) {
        var _this = this,
